...
|----------|----------|----------|--------------|
| **第一阶段** | News Monitor → AI Brief → 邮件订阅 | Ingest + Aggregation + subscriptions + LLM brief + 邮件通知 | **部分完成**（见 §九） |
| **第二阶段** | AI Research（竞品/品牌/行业监控） | pgvector + tracking_threads + 实体对比 + 定期报告 | **MVP 已开工** |
...
- [x] News Monitor 底座：RSS 入库 + PG digest + Platform API + 前端优先读 `/platform`
- [ ] News Monitor：前端全面接 Aggregate API（仍有 RSS fallback）
- [x] AI Brief：按需简报 pipeline（`brief-service` + HXXBOT `qa_session` + `briefs` 表）
- [ ] AI Brief：定时调度（cron / subscription worker 每日触发）
- [x] 邮件订阅 MVP：users + subscriptions CRUD + 匹配引擎 + HXXBOT 邮件投递
- [x] 订阅语言：deliveryLang + 源语言过滤；不一致时翻译缓存（DB + OSS 分类目录）
- [x] 管理后台：可订阅项预设 + 用户 + 订阅规则（`/admin`）
- [x] Platform 文件日志 + 管理后台「系统日志」查看（`logs/{service}/{date}.log`）
- [ ] 邮件订阅：用户认证 JWT + 前端用户自助订阅页

### 商业 Phase 2 — AI Research（MVP 已落地）

- [x] pgvector `news_embeddings` + 本地 embedding worker（`@xenova/transformers`）
- [x] Redis Stream 队列（`REDIS_URL`）+ `platform:embed` worker
- [x] `monitor_profiles` / `entities` / `tracking_threads` Repository
- [x] Platform API：`/platform/v1/research/*`、`/platform/v1/embedding/run`
- [ ] 前端 Research 面板 / 定期报告调度
- [ ] 品牌情感、行业 taxonomy 深化
