...
- 数据源 catalog：`packages/platform-core/src/integration-provider-catalog.ts`
- 引擎 catalog：`packages/platform-core/src/engine-catalog.ts`
- 采集绑定：`packages/platform-core/src/ingest-bindings-repository.ts`
- Ingest 插件：`packages/platform-core/src/ingest-plugins/`
...
| 管理后台 | 数据库表 | 是什么 |
|----------|----------|--------|
| **数据源配置** | `integration_providers` | 第三方 API / 披露站点的 **Base URL + API Key**（地图、行情、发邮件、巨潮根 URL 等**业务数据源凭证**） |
| **采集引擎** | `engines` | 通用采集引擎凭证（Firecrawl 等，`engine_type` 区分类型）；**不是**业务数据源 |
| **采集绑定** | `data_source_ingest_bindings` | 数据源 ↔ 引擎 ↔ ingest 插件 key（如 `cninfo` → `firecrawl` + `cninfo-disclosure`） |
...
**披露类数据源**（如 `cninfo`）只配置站点根 URL，不配置 Firecrawl Key；抓取凭证在 **采集引擎** 菜单的 `engines` 表，通过 `data_source_ingest_bindings` 引用。

### 2.1.1 采集引擎（engines）与绑定（data_source_ingest_bindings）

| 表 | 示例 | 说明 |
|----|------|------|
| `engines` | `firecrawl`（`engine_type=crawl`） | 通用引擎凭证；Admin 侧栏「采集引擎」 |
| `data_source_ingest_bindings` | `cninfo` → `firecrawl` + `cninfo-disclosure` | 数据源使用哪个引擎、由哪个 ingest 插件执行 |

内置映射（migration 025 + seed）：

- `cninfo` / `firecrawl` / `cninfo-disclosure`

采集逻辑统一在 `packages/platform-core/src/ingest-plugins/` 注册；Job handler 由注册表自动生成并 dispatch 到插件。RSS/行情等**实现**仍在 `rss-ingest.ts`、`equity-ingest.ts` 等模块，插件层为薄封装。

| 插件 key | Job handler | 实现模块 |
|----------|-------------|----------|
| `rss-ingest-fast` | 同名 | `rss-ingest.ts` |
| `rss-ingest-full` | 同名 | `rss-ingest.ts` |
| `stock-news-ingest` | 同名 | `equity-ingest.ts` |
| `cninfo-disclosure` | `disclosure-ingest-cn` | `enterprise-graph/cninfo/*` 流水线（需引擎绑定） |

### 2.1.2 巨潮披露采集（CNINFO Phase 1）

- **主数据**：每条公告 list 结果 upsert `listed_companies` + `listed_securities`（含 `country_code` / `market` / `exchange` / `region_keys`）。
- **幂等**：`company_filings` 唯一键 `(workspace_id, source, source_doc_id)`；默认增量 + 已采 skip；`payload.force` / `recollect` / `since` 可重采。
- **external_key**：公司 `cn:SSE:600519`；公告 `filing:cn:cninfo:{announcementId}`。
- **解析降级**：direct HTTP → Firecrawl → pdf-parse；状态写入 `parse_status` / `parse_method`。
- **Checkpoint**：`job_checkpoints` handler `disclosure-ingest-cn`，stats 含 `lastAnnTime`。

```json
{ "handlerKey": "disclosure-ingest-cn", "payload": { "lookbackDays": 7 } }
{ "handlerKey": "disclosure-ingest-cn", "payload": { "recollect": "failed" } }
```
