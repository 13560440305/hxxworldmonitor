...
export interface AdminStats {
  ...
  database?: { enabled: boolean; connected: boolean };
  redis?: { enabled: boolean; ok?: boolean; error?: string };
  adminAuth: boolean;
...
export async function loginAdmin(email: string, password: string): Promise<{ email: string }> {
  if (!isPlatformApiConfigured()) {
    throw new Error(
      '请在仓库根目录 .env.local 配置 VITE_PLATFORM_API_URL=http://localhost:8787 并重启开发服务器（npm run admin:dev 或 npm run dev）',
    );
  }
  let resp: Response;
  try {
...
export async function fetchJobCheckpoints(): Promise<JobCheckpointRow[]> {
  const data = await parseJson<{ checkpoints: JobCheckpointRow[] }>(
    await adminFetch('/v1/admin/jobs/checkpoints'),
  );
  return data.checkpoints ?? [];
}

export async function fetchKgDagStatus(): Promise<KgDagStatus> {
  const data = await parseJson<{ dag: KgDagStatus }>(
    await adminFetch('/v1/admin/jobs/dag-status'),
  );
  return data.dag;
}
