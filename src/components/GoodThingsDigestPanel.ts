...
export class GoodThingsDigestPanel extends Panel {
  ...
  constructor() {
    ...
    this.content.innerHTML = `<p class="digest-placeholder">${t('happy.digest.loading')}</p>`;
  }
  ...
  public async setStories(items: NewsItem[]): Promise<void> {
    ...
    if (top5.length === 0) {
      this.content.innerHTML = `<p class="digest-placeholder">${t('happy.digest.empty')}</p>`;
      this.cardElements = [];
      return;
    }
    ...
    for (let i = 0; i < top5.length; i++) {
      ...
      card.innerHTML = `
        ...
        <div class="digest-card-body">
          ...
          <p class="digest-card-summary digest-card-summary--loading">${t('happy.digest.summarizing')}</p>
        </div>
      `;
